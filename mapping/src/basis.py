...
def Basis(config, State, verbose=True, multi_mode=False, *args, **kwargs):
    ...
    if config.BASIS is None:
        ...
        elif config.BASIS.super=='BASIS_MIOST_JAX':
            return Basis_miost_jax(config,State,multi_mode=multi_mode)
        
        elif config.BASIS.super=='BASIS_IT':
            return Basis_it(config, State)

        elif config.BASIS.super=='BASIS_IT_OLD':
            return Basis_it_old(config, State)
...
class Basis_bmaux:
    ...
    def set_basis(self,time,return_q=False,**kwargs):
        ...
        if self.facQ_aux_path is not None: 
            facQ_aux = xr.open_dataset(self.facQ_aux_path,decode_times=False)
            # Convert longitude 
            if np.sign(facQ_aux['lon'].data.min())==-1 and self.lon_unit=='0_360':
                facQ_aux = facQ_aux.assign_coords({'lon':(('lon', facQ_aux['lon'].data % 360))})
            elif (np.sign(facQ_aux['lon'].data.min())>=0 or facQ_aux['lon'].data.max()>180) and self.lon_unit=='-180_180':
                facQ_aux = facQ_aux.assign_coords({'lon':(('lon', (facQ_aux['lon'].data + 180) % 360 - 180 ))})
            facQ_aux = facQ_aux.sortby(facQ_aux['lon'])    
            psd_ref = facQ_aux['psd_ref']
            psd_diff = facQ_aux['psd_diff']
        ...
        for iff in range(nf):
            ...
            for P in range(NP[iff]):
                ...
                if self.facQ_aux_path is not None: 
                    nsr_tmp_values1 = psd_ref.interp(wavenumber=ff[iff], lon=elon2.ravel(), lat=elat2.ravel()).values
                    nsr_tmp_values2 = psd_diff.interp(wavenumber=ff[iff], lon=elon2.ravel(), lat=elat2.ravel()).values
                    nsr_tmp_values = np.maximum(np.minimum(1,nsr_tmp_values2/nsr_tmp_values1),1e-05)
                    #nsr_tmp_values[nsr_tmp_values>0.5] = 1
                    nsr_tmp = np.nanmean(nsr_tmp_values) if not np.all(np.isnan(nsr_tmp_values)) else 1 
                    facQaux[iff].append(nsr_tmp) 
        ...
        for iff in range(nf):
            ...
            for it in range(num_it):
                for P in range(NP[iff]):
                    ...
                    Q_tmp = np.sqrt(facQ*Q_tmp**2)  # Multiply after NaN check

                    if self.facQ_aux_path is not None:
                        Q_tmp = np.sqrt(facQaux[iff][P]*Q_tmp**2)

                    if 1/ff[iff]>l_largescale: 
                        Q_tmp *= facQ_largescale 
...
class Basis_miost:
    ...
    def set_basis(self, time, return_q=False,**kwargs):
        ...
        # Convert longitudes
        if np.sign(daPSDS['lon'].data.min())==-1 and self.lon_unit=='0_360':
            daPSDS = daPSDS.assign_coords({'lon':(('lon', daPSDS['lon'].data % 360))})
        elif (np.sign(daPSDS['lon'].data.min())>=0 or daPSDS['lon'].data.max()>180) and self.lon_unit=='-180_180':
            daPSDS = daPSDS.assign_coords({'lon':(('lon', (daPSDS['lon'].data + 180) % 360 - 180 ))})
        if np.sign(daC['lon'].data.min())==-1 and self.lon_unit=='0_360':
            daC = daC.sel(lon=slice(-180,179.99))
            daC = daC.assign_coords({'lon':(('lon', daC['lon'].data % 360))})
        elif (np.sign(daC['lon'].data.min())>=0 or daC['lon'].data.max()>180) and self.lon_unit=='-180_180':
            daC = daC.sel(lon=slice(0,359.99))
            daC = daC.assign_coords({'lon':(('lon', (daC['lon'].data + 180) % 360 - 180 ))})
        daPSDS = daPSDS.sortby(daPSDS['lon'])    
        daC = daC.sortby(daC['lon'])    
        daC = daC.T
        ...
        ENSLAT = [None]*nf # Ensemble of latitudes of the center of each wavelets
        tdec_max = [None]*nf
        norm_fact = list() # integral of the time component (for normalization)
        ...
        for iff in range(nf):
            ...
            enst.append(list())
            tdec.append(list())
            norm_fact.append(list())
            Cb1.append(list())

            for P in range(NP[iff]):
                enst[-1].append(list())
                tdec[-1].append(list()) 
                norm_fact[-1].append(list()) 
                Cb1[-1].append(list())
                ...
                elat = np.linspace(ENSLAT[iff][P]-dlat, ENSLAT[iff][P]+dlat,10)
                elon2, elat2 = np.meshgrid(elon,elat)
                Ctmp = daC.interp(lat=elat2.flatten(), lon=elon2.flatten()).values
                ...
                if tdecp<tdec[-1][-1] or np.isnan(tdec[-1][-1]): 
                    tdec[-1][-1] = tdecp
                ...
                for i in range(tt.size-1):
                    tmp[i+1] = tmp[i] + self.window(tt[i]/tdec[iff][P])*(tt[i+1]-tt[i])
                norm_fact[-1][-1] = tmp.max()
            
                # Time coordinates
                enst[-1][-1] = np.arange(-tdec[iff][P]/self.facnlt,deltat+tdec[iff][P]/self.facnlt , tdec[iff][P]/self.facnlt)
        ...
        for iff in range(nf):
            ...
            for P in range(NP[iff]):
                ...
                if self.wavetest[iff][P]:
                    std_tmp = (PSDLOC*ff[iff]**2 * self.facQ * np.exp(-3*(self.cutRo * Ro*ff[iff])**3))**.5 
                else:
                    std_tmp = 1e-15
                std[iff][P] = std_tmp
        ...
        for iff in range(nf):
            ...
            for it in range(enst_same_dim[iff].shape[1]):
                for P in range(NP[iff]):
                    self.P_wavebounds[iff][P] = iwave+1
                    if np.isnan(enst_same_dim[iff][P,it]):
                        Q_tmp = 1e-15 # Not zero otherwise a ZeroDivisionError exception will be raised
                    else:
                        Q_tmp = std[iff][P]
                    Qf_list.append(Q_tmp * np.ones(2 * ntheta))
                    _nwavef += 2*ntheta
...
class Basis_miost_jax(Basis_miost):
    def __init__(self, config, State, multi_mode=False):
        super().__init__(config, State, multi_mode=multi_mode)
...
class Basis_it:
                        ...
                        if hasattr(self.sigma_B_bc,'__len__'):
                            if len(self.sigma_B_bc)==self.Nwaves:
                                ...
                                for iw in range(self.Nwaves):
                                    ...
                                    Q[self.slice_params[name]][slicew]=self.sigma_B_bc[iw]/(self.facns*self.facnlt)
                            else:
                            ...
                            Q[self.slice_params[name]]=self.sigma_B_bc/self.facns/self.facnlt
    ...
    def operg(self,t,X,State=None):
        ...
        if State is not None:
            for name in self.name_params : 
                ...
                if name == "hbcx" : 
                    State['hbcx'] = np.concatenate((np.expand_dims(phi[self.slice_params_phys["hbcS"]].reshape(self.shape_params_phys["hbcS"]),axis=1),
                ...
                # - Height boundary conditions hbcy - #
                    ...
                    State['hbcy'] = np.concatenate((np.expand_dims(phi[self.slice_params_phys["hbcE"]].reshape(self.shape_params_phys["hbcE"]),axis=1),
                ...
                # - Equivalent Height He - OR - Internal Tide Generation itg - #
                    ...
                    State[name] = phi[self.slice_params_phys[name]].reshape(self.shape_params_phys[name])
        else: 
            return phi
    ...
    def operg_transpose(self,t,phi=None,adState=None):
        ...
        if phi is not None: # If provided through phi ndarray argument 
            ...
            for name in self.name_params:
                if name == "hbcx" : 
                    param["hbcS"] = adState[name][:,0,:,:,:].reshape(self.shape_params_phys["hbcS"])
                    param["hbcN"] = adState[name][:,1,:,:,:].reshape(self.shape_params_phys["hbcN"])
                elif name == "hbcy" : 
                    param["hbcE"] = adState[name][:,0,:,:,:].reshape(self.shape_params_phys["hbcE"])
                    param["hbcW"] = adState[name][:,1,:,:,:].reshape(self.shape_params_phys["hbcW"])
                else :
                    param[name] = adState[name].reshape(self.shape_params_phys[name])
        else: 
...
class Basis_it_old:
    ...
    def set_basis(self,time,return_q=False,**kwargs):
        ...
        self.He_xy_gauss = jnp.array(He_xy_gauss)
        self.He_t_gauss = jnp.array(He_t_gauss)
        ...
        self.bc_S_gauss = jnp.array(bc_S_gauss)
        self.bc_N_gauss = jnp.array(bc_N_gauss)
        self.bc_E_gauss = jnp.array(bc_E_gauss)
        self.bc_W_gauss = jnp.array(bc_W_gauss)
        ...
        for i,time0 in enumerate(ENST_bc):
            iobs = np.where(abs(time-time0) < self.T_bc)
            bc_t_gauss[i,iobs] = mywindow(abs(time-time0)[iobs]/self.T_bc)
        self.bc_t_gauss = jnp.array(bc_t_gauss)
    ...
    def _operg(self, t, X):
        ...
        indt = jnp.where(self.time == t, size=1)[0][0] 

        He = jnp.tensordot(
            jnp.tensordot(X_He,self.He_xy_gauss,(1,0)),
                                self.He_t_gauss[:,indt],(0,0))
    
        hbcx = jnp.zeros(self.shapehbcx_phys)
        hbcy = jnp.zeros(self.shapehbcy_phys)
    
        hbcx = hbcx.at[:,0].set(jnp.tensordot(
            jnp.tensordot(X_bcS,self.bc_S_gauss,(-1,0)),
                                 self.bc_t_gauss[:,indt],(-2,0)))
        hbcx = hbcx.at[:,1].set(jnp.tensordot(
            jnp.tensordot(X_bcN,self.bc_N_gauss,(-1,0)),
                                 self.bc_t_gauss[:,indt],(-2,0)))
        hbcy = hbcy.at[:,0].set(jnp.tensordot(
            jnp.tensordot(X_bcE,self.bc_E_gauss,(-1,0)),
                                 self.bc_t_gauss[:,indt],(-2,0)))
        hbcy = hbcy.at[:,1].set(jnp.tensordot(
            jnp.tensordot(X_bcW,self.bc_W_gauss,(-1,0)),
                                 self.bc_t_gauss[:,indt],(-2,0)))

        phi = jnp.concatenate((He.flatten(), hbcx.flatten(), hbcy.flatten()))

        return phi

    def _operg_reduced(self, t, phi):
        """
        Project a 2D physical space field back to the reduced space.

        Parameters:
            t: Current time
            phi: 2D physical space field to project back.
    ...
    def operg(self, t, X, State=None):
        ...
        if State is not None:
            State['He'] = phi[self.sliceHe_phys].reshape(self.shapeHe_phys)
            State['hbcx'] = phi[self.slicehbcx_phys].reshape(self.shapehbcx_phys)
            State['hbcy'] = phi[self.slicehbcy_phys].reshape(self.shapehbcy_phys)
        else:
            return phi


    def operg_transpose(self, t, adState=None):
        """
            Project to reduced space
        """
        
        He = +adState['He'].reshape(self.shapeHe_phys)
        hbcx = +adState['hbcx'].reshape(self.shapehbcx_phys)
        hbcy = +adState['hbcy'].reshape(self.shapehbcy_phys)
        adState['He'] *= 0
        adState['hbcx'] *= 0
        adState['hbcy'] *= 0

        phi = jnp.concatenate((He.flatten(),hbcx.flatten(),hbcy.flatten()))
        adX = self._operg_reduced_jit(t, phi)
