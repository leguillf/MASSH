...
class Qgm:
    ...
    def __init__(self, dx=None, dy=None, dt=None, SSH=None, c=None, upwind=3,
        ...
        self.one_step_jit = jit(self.one_step, static_argnums=1)
        self.one_step_for_scan_jit = jit(self.one_step_for_scan)
        self.step_jit = jit(self.step, static_argnums=4)
    ...
    def one_step(self, h0, var0, hb, varb, Nudg_term=None, way=1):
        ...
        # Nudging
        #if Nudg_term is not None:
        #    q1 += Nudg_term['rv'] - (self.g*self.f)/self.c**2 * Nudg_term['ssh']
    ...
    def one_step_for_scan(self,X0,X):
        ...
        h1, var1, hb, varb, Nudg_term, way = X0
        h1, var1 = self.one_step_jit(h1, var1, hb, varb, Nudg_term, way)
        X = (h1, var1, hb, varb, Nudg_term, way)

        return X,X

    def step(self, X0, Xb, Nudg_term=None, way=1, nstep=1):
        ...
        # Boundary conditions
        h0 = self.Wbc*hb + (1-self.Wbc)*h0
        q0 = self.Wbc*qb + (1-self.Wbc)*q0
        ...
        # Time propagation
        X1, _ = scan(self.one_step_for_scan_jit, init=(h1, var1, hb, varb, Nudg_term, way), xs=jnp.zeros(nstep))
        h1, var1, hb, varb, Nudg_term, way = X1
        ...
        if Nudg_term is not None:
            if len(var1.shape)==3:
                ...
                q1 = +var1
            q1 += (1-self.Wbc) * (Nudg_term['rv'] - (self.g*self.f)/self.c**2 * Nudg_term['ssh'])
            # Elliptical inversion  
            h1 = self.pv2h_jit(q1, hb, qb)
