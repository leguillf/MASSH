...
def main(param_file):
    ...
    config['PICKLE_FILE_NAME'] = param_file.rsplit('/', 1)[-1].rsplit('_step1.', 1)[0]
...
def run_miost(config, comm=None, rank=0, size=1):
    ...
    file_pickle = config['PATH']['OUTPUT']+"/obs_val_"+config['PICKLE_FILE_NAME']+"_proc"+str(rank)+".p"
    import pickle
    obs_val_loc_expl = pickle.load( open( file_pickle, "rb" ) )
    logging.info('Miost step 1 %s loaded', file_pickle)
    obs_val_loc -= obs_val_loc_expl[0]
