...
class Swm: 
    ...
    def _compute_w1_IT_new(self,t,He,h_SN,h_WE):
        ...
        _omega, _bctheta, _coords = np.meshgrid(
            self.omegas, self.bc_theta, self.Xv[0, :], indexing="ij"
        )

        _Xv, _Yv, _fS, _HeS = self.Xv[0, :], self.Yv[0, :], (self.f[0,:]+self.f[1,:])/2, (He[0,:]+He[1,:])/2

        _Xv = jnp.broadcast_to(_Xv, _coords.shape)
        _Yv = jnp.broadcast_to(_Yv, _coords.shape)
        _fS = jnp.broadcast_to(_fS, _coords.shape)
        _HeS = jnp.broadcast_to(_HeS, _coords.shape)

        k = jnp.sqrt((_omega**2-_fS**2)/(self.g*_HeS))
        kx = jnp.sin(_bctheta) * k
        ky = jnp.cos(_bctheta) * k
        kxy = kx*_Xv + ky*_Yv

        h = h_SN[:,0,0,:]* jnp.cos(_omega*t-kxy)  +\
                        h_SN[:,0,1,:]* jnp.sin(_omega*t-kxy) 

        v = self.g/(_omega**2-_fS**2)*( \
                    h_SN[:,0,0,:]* (_omega*ky*jnp.cos(_omega*t-kxy) \
                                - _fS*kx*jnp.sin(_omega*t-kxy)
                                    ) +\
                    h_SN[:,0,1,:]* (_omega*ky*jnp.sin(_omega*t-kxy) \
                                + _fS*kx*jnp.cos(_omega*t-kxy)
                                    )
                        )

        w1S = v.sum(axis=(0,1)) + jnp.sqrt(self.g/_HeS[0,0,:]) * h.sum(axis=(0,1))
        ...
        _omega, _bctheta, _coords = np.meshgrid(
            self.omegas, self.bc_theta, self.Xv[-1,:], indexing="ij"
        )

        _Xv, _Yv, _fN, _HeN = self.Xv[-1,:], self.Yv[-1,:], (self.f[-1,:]+self.f[-2,:])/2, (He[-1,:]+He[-2,:])/2

        _Xv = jnp.broadcast_to(_Xv, _coords.shape)
        _Yv = jnp.broadcast_to(_Yv, _coords.shape)
        _fN = jnp.broadcast_to(_fN, _coords.shape)
        _HeN = jnp.broadcast_to(_HeN, _coords.shape)

        k = jnp.sqrt((_omega**2-_fN**2)/(self.g*_HeN))
        kx = jnp.sin(_bctheta) * k
        ky = -jnp.cos(_bctheta) * k
        kxy = kx*_Xv + ky*_Yv

        h = h_SN[:,1,0,:]* jnp.cos(_omega*t-kxy)  +\
                        h_SN[:,1,1,:]* jnp.sin(_omega*t-kxy) 

        v = self.g/(_omega**2-_fN**2)*( \
                    h_SN[:,1,0,:]* (_omega*ky*jnp.cos(_omega*t-kxy) \
                                - _fN*kx*jnp.sin(_omega*t-kxy)
                                    ) +\
                    h_SN[:,1,1,:]* (_omega*ky*jnp.sin(_omega*t-kxy) \
                                + _fN*kx*jnp.cos(_omega*t-kxy)
                                    )
                        )

        w1N = v.sum(axis=(0,1)) - jnp.sqrt(self.g/_HeN[0,0,:]) * h.sum(axis=(0,1))
        ...
        _omega, _bctheta, _coords = np.meshgrid(
            self.omegas, self.bc_theta, self.Xu[:,0], indexing="ij"
        )

        # Split _coords into _Xv and _Yv
        _Xu, _Yu, _fW, _HeW = self.Xu[:,0], self.Yu[:,0], (self.f[:,0]+self.f[:,1])/2, (He[:,0]+He[:,1])/2

        # Expand _Xv and _Yv to match _coords' shape
        _Xu = jnp.broadcast_to(_Xu, _coords.shape)
        _Yu = jnp.broadcast_to(_Yu, _coords.shape)
        _fW = jnp.broadcast_to(_fW, _coords.shape)
        _HeW = jnp.broadcast_to(_HeW, _coords.shape)

        k = jnp.sqrt((_omega**2-_fW**2)/(self.g*_HeW))
        kx = jnp.cos(_bctheta) * k
        ky = jnp.sin(_bctheta) * k
        kxy = kx*_Xu + ky*_Yu

        h = h_WE[:,0,0,:]* jnp.cos(_omega*t-kxy)  +\
                        h_WE[:,0,1,:]* jnp.sin(_omega*t-kxy) 

        v = self.g/(_omega**2-_fW**2)*( \
                    h_WE[:,0,0,:]* (_omega*kx*jnp.cos(_omega*t-kxy) \
                                + _fW*ky*jnp.sin(_omega*t-kxy)
                                    ) +\
                    h_WE[:,0,1,:]* (_omega*kx*jnp.sin(_omega*t-kxy) \
                                - _fW*ky*jnp.cos(_omega*t-kxy)
                                    )
                        )

        w1W = v.sum(axis=(0,1)) + jnp.sqrt(self.g/_HeW[0,0,:]) * h.sum(axis=(0,1))

        
        # East
        _omega, _bctheta, _coords = np.meshgrid(
            self.omegas, self.bc_theta, self.Xu[:,-1], indexing="ij"
        )

        # Split _coords into _Xv and _Yv
        _Xu, _Yu, _fE, _HeE = self.Xu[:,-1], self.Yu[:,-1], (self.f[:,-1]+self.f[:,-2])/2, (He[:,-1]+He[:,-2])/2

        # Expand _Xv and _Yv to match _coords' shape
        _Xu = jnp.broadcast_to(_Xu, _coords.shape)
        _Yu = jnp.broadcast_to(_Yu, _coords.shape)
        _fE = jnp.broadcast_to(_fE, _coords.shape)
        _HeE = jnp.broadcast_to(_HeE, _coords.shape)

        k = jnp.sqrt((_omega**2-_fE**2)/(self.g*_HeE))
        kx = -jnp.cos(_bctheta) * k
        ky = jnp.sin(_bctheta) * k
        kxy = kx*_Xu + ky*_Yu

        h = h_WE[:,1,0,:]* jnp.cos(_omega*t-kxy)  +\
                        h_WE[:,1,1,:]* jnp.sin(_omega*t-kxy) 

        v = self.g/(_omega**2-_fE**2)*( \
                    h_WE[:,1,0,:]* (_omega*kx*jnp.cos(_omega*t-kxy) \
                                + _fE*ky*jnp.sin(_omega*t-kxy)
                                    ) +\
                    h_WE[:,1,1,:]* (_omega*kx*jnp.sin(_omega*t-kxy) \
                                - _fE*ky*jnp.cos(_omega*t-kxy)
                                    )
                        )

        w1E = v.sum(axis=(0,1)) - jnp.sqrt(self.g/_HeE[0,0,:]) * h.sum(axis=(0,1))
