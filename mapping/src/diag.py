...
That could be due to non regular grid or bad written netcdf file')
        ...
        exp = exp.assign_coords({self.name_exp_lon:exp[self.name_exp_lon]%360})
...
class Diag_ose():
    ...
    def __init__(self,config,State):
        ...
        ref = ref.assign_coords({self.name_ref_time:ref[self.name_ref_time]})
        ref = ref.swap_dims({ref[self.name_ref_time].dims[0]:self.name_ref_time})
        lon_ref = ref[self.name_ref_lon] % 360
        lat_ref = ref[self.name_ref_lat]
        ref = ref.where((lat_ref >= self.lat_min) & (lat_ref <= self.lat_max), drop=True)
        ref = ref.where((lon_ref >= self.lon_min%360.) & (lon_ref <= self.lon_max%360.), drop=True)
        try:
            ref = ref.sel(
...
That could be due to non regular grid or bad written netcdf file')
    ...
    def _regrid_geo(self, lon, lat, time, var):
        ...
        # Define source grid
        x_source_axis = pyinterp.Axis(lon%360, is_circle=False)
        y_source_axis = pyinterp.Axis(lat)
        z_source_axis = pyinterp.TemporalAxis(time)
        ...
        var_interp = pyinterp.trivariate(grid_source,
                                        self.ref[self.name_ref_lon].values%360, 
        ...
        return xr.DataArray(
            ...
            coords={self.name_ref_time: (self.name_ref_time, self.ref[self.name_ref_time].values),
                    self.name_ref_lon: (self.name_ref_time, self.ref[self.name_ref_lon].values%360), 
    ...
    def _regrid_unstructured(self, lon, lat, time, var):
        ...
        lon1d = np.arange(np.nanmin(lon),np.nanmax(lon)+dlon,dlon)%360
