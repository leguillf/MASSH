...
class Variational:
    ...
    def grad(self,X0): 
        ...
        for i in reversed(range(0,len(self.checkpoints)-1)):
            ...
            # if i == int(len(self.checkpoints)/2):
            #     plt.figure()
            #     plt.pcolormesh(adState.params["He"]+0.95+adState.params["He_offset"])
        ...
        # self.basis.operg(0/3600/24,adX,State=State)
        # plt.figure()
        # plt.pcolormesh(State.var["ssh_bm"])
        # plt.title('init state parameters at the end of gradient function evaluation')
        # plt.colorbar()
        # plt.show()
