        ...
        for iff in range(self.nf):
            for P in range(self.NP[iff]):
                    ...
                    if (iff,P) in self.indx:
                        ...
                        iobs = np.where((np.abs((np.mod(lon - self.ENSLON[iff][P]+180,360)-180) / self.km2deg * np.cos(self.ENSLAT[iff][P] * np.pi / 180.)) <= self.DX[iff]) &
                                    (np.abs((lat - self.ENSLAT[iff][P]) / self.km2deg) <= self.DX[iff]))[0]
                        xx = (np.mod(lon[iobs] - self.ENSLON[iff][P]+180,360)-180) / self.km2deg * np.cos(self.ENSLAT[iff][P] * np.pi / 180.) 
                        yy = (lat[iobs] - self.ENSLAT[iff][P]) / self.km2deg
                        # Spatial tapering shape of the wavelet 
        ...
        for P in range(self.NP):
            ...
            for it in range(len(enstloc)):
                ...
                if iobs.shape[0] > 0:                        
                    ...
                    if nobs > 0:
                        tt2 = diff[iobs2]
                        fact = mywindow_flux(tt2 / self.tdec[P])
                        t = np.linspace(-self.tdec[P],self.tdec[P])
                        I =  np.sum(mywindow(t/self.tdec[P]))*(t[1]-t[0])
                        fact /= I 
