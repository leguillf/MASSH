...
class Model_sw1l_jax(M):
    ...
    def step(self,State,nstep=1,t=0):
        ...
        np.save("/home/bellemva/MASSH_dev/mapping/examples/X1.npy",X1)
...
## VERSION MODIFIEE DE MODSW1L DE FLO ##
...
class Model_sw1l(M):
    ...
    def __init__(self,config,State):
        ...
        # Initializing slice information of parameters
        idx = 0 
        self.slice_params={}
        for param in ['He','hbcx','hbcy','itg'] : 
            self.slice_params[param] = slice(idx, idx + np.prod(self.shape_params[param]))
            idx += np.prod(self.shape_params[param])
    ...
    def step(self,State,nstep=1,t=0):
        ...
        np.save("/home/bellemva/MASSH/mapping/examples/X1.npy",X1)
    ...
    def _jstep(self,X0):
        ...
        # - Equivalent Height - # 
        He = np.zeros(self.shape_params['He'])+self.Heb
        ...
        _X1 = jnp.concatenate((
            +X1[:self.swm.nstates], #State 
            He.flatten(), # He parameter 
            jnp.concatenate((w1S,w1N,w1W,w1E)), # BC parameter 
            rhs_itg.flatten() # ITG parameter
        ))
    ...
    def step_tgl(self,dState,State,nstep=1,t=0):
        ...
        if State.params is not None:
            for param in ['He','hbcx','hbcy','itg'] :
                params = +State.getparams(param,vect=True)
                X0 = np.concatenate((X0,params))
                dparams = +dState.getparams(param,vect=True)
    ...
    def step_adj(self,adState, State, nstep=1,t=0):
        ...
        if State.params is not None:
            for param in ['He','hbcx','hbcy','itg'] :
                params = +State.getparams(param,vect=True)
                X0 = np.concatenate((X0,params))
                adparams = +adState.getparams(param,vect=True)
        ...
        adState.params['He'] = +adparams[self.slice_params['He']].reshape(self.shape_params['He'])
        adState.params['hbcx'] = +adparams[self.slice_params['hbcx']].reshape(self.shape_params['hbcx'])
        adState.params['hbcy'] = +adparams[self.slice_params['hbcy']].reshape(self.shape_params['hbcy'])
        adState.params['itg'] = +adparams[self.slice_params['itg']].reshape(self.shape_params['itg'])
